...
// must match the base sprite grid.
...
for (const [name, clip] of Object.entries(CLIPS)) {
  ...
  for (const frame of allFrames(clip)) {
    if (!clip.stages && frame.length !== SPRITE_H * res) throw new Error(`clip ${name}: frame has ${frame.length} rows`);
    for (const row of frame) {
      if (!clip.stages && row.length !== SPRITE_W * res) throw new Error(`clip ${name}: row "${row}" is ${row.length} wide`);
...
export { CLIPS, PALETTES, GLYPHS, SPRITE_W, SPRITE_H, SOCCER };
